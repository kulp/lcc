...
#define sfoldcnst(OP) \
	...
	&& r->u.v.i >= 0 && r->u.v.i < TARGET_CHAR_BIT*l->type->size) \
...
Tree simplify(int op, Type ty, Tree l, Tree r) {
	...
	switch (op) {
		...
		case CVP+P:
			...
			break;
		case CVI+U:
			xcvtcnst(I,l->u.v.i,ty,u,((unsigned long)l->u.v.i)&ones(TARGET_CHAR_BIT*ty->size));
			break;
		case CVU+U:
			xcvtcnst(U,l->u.v.u,ty,u,l->u.v.u&ones(TARGET_CHAR_BIT*ty->size));
			break;
		...
		case BAND+U:
			foldcnst(U,u,&);
			commute(r,l);
			identity(r,l,U,u,ones(TARGET_CHAR_BIT*ty->size));
		...
		case BAND+I:
			foldcnst(I,i,&);
			commute(r,l);
			identity(r,l,I,i,ones(TARGET_CHAR_BIT*ty->size));
		...
		case OR+I:
			...
			break;
		case BCOM+I:
			ufoldcnst(I,cnsttree(ty, (long)extend((~l->u.v.i)&ones(TARGET_CHAR_BIT*ty->size), ty)));
			idempotent(BCOM+U);
			break;
		case BCOM+U:
			ufoldcnst(U,cnsttree(ty, (unsigned long)((~l->u.v.u)&ones(TARGET_CHAR_BIT*ty->size))));
			idempotent(BCOM+U);
			break;
		case BOR+U:
			foldcnst(U,u,|);
		...
		case LSH+I:
			...
			if (r->op == CNST+I && (r->u.v.i >= TARGET_CHAR_BIT*ty->size || r->u.v.i < 0)) {
		...
		case RSH+I:
			...
			&& r->u.v.i >= 0 && r->u.v.i < TARGET_CHAR_BIT*l->type->size) {
				...
				if (l->u.v.i < 0)
					n |= ~0UL<<(TARGET_CHAR_BIT*l->type->size - r->u.v.i);
